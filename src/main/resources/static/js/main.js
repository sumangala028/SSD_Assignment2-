...
function onFileSelect(e) {
	...
	for (var i = 0; i < fileCount; i++) {
		var file = files[i];
		output.push(file.name, ' (', bytesToSize(file.size), ', Modified on ', file.lastModifiedDate
			.toLocaleDateString(), ')');
		output.push('<br/>');
		selectedFileCount += file.size;
	}
	document.getElementById('selectedFiles').innerHTML = "Selected Files : " + output.join('');
